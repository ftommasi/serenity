...
AbstractView::AbstractView()
    : m_sort_order(SortOrder::Ascending), m_selection(*this) {
  REGISTER_BOOL_PROPERTY("activates_on_selection", activates_on_selection,
                         set_activates_on_selection);
  REGISTER_BOOL_PROPERTY("editable", is_editable, set_editable);
  REGISTER_BOOL_PROPERTY("searchable", is_searchable, set_searchable);
  REGISTER_ENUM_PROPERTY("selection_behavior", selection_behavior,
                         set_selection_behavior, SelectionBehavior,
                         {SelectionBehavior::SelectItems, "SelectItems"},
                         {SelectionBehavior::SelectRows, "SelectRows"});
  REGISTER_ENUM_PROPERTY("selection_mode", selection_mode, set_selection_mode,
                         SelectionMode,
  ...
  REGISTER_BOOL_PROPERTY("tab_key_navigation_enabled",
                         is_tab_key_navigation_enabled,
                         set_tab_key_navigation_enabled);
  REGISTER_BOOL_PROPERTY("draw_item_text_with_shadow",
                         does_draw_item_text_with_shadow,
                         set_draw_item_text_with_shadow);
...
void AbstractView::model_did_update(unsigned int flags) {
  if (!model() || (flags & GUI::Model::InvalidateAllIndices)) {
    ...
    if (!model()->is_within_range(m_drop_candidate_index))
      m_drop_candidate_index = {};
    selection().remove_all_matching(
        [this](auto &index) { return !model()->is_within_range(index); });
...
void AbstractView::clear_selection() { m_selection.clear(); }
...
void AbstractView::did_scroll() { update_edit_widget_position(); }

void AbstractView::update_edit_widget_position() {
  if (!m_edit_widget)
    return;
  m_edit_widget->set_relative_rect(m_edit_widget_content_rect.translated(
...
void AbstractView::begin_editing(ModelIndex const &index) {
  ...
  m_edit_widget_content_rect =
      editing_rect(index).translated(frame_thickness(), frame_thickness());
...
void AbstractView::activate_selected() {
  ...
  selection().for_each_index([this](auto &index) { on_activation(index); });
}
...
NonnullRefPtr<Gfx::Font>
AbstractView::font_for_index(ModelIndex const &index) const {
  if (!model())
    return font();
...
void AbstractView::mousedown_event(MouseEvent &event) {
  ...
  if (!index.is_valid()) {
    ...
    set_cursor(index, SelectionUpdate::Shift);
             ...
             m_selection.contains(index) &&
             !m_model->drag_data_type().is_null()) {
    // We might be starting a drag, so don't throw away other selected items
    // yet.
    m_might_drag = true;
...
void AbstractView::mousemove_event(MouseEvent &event) {
  ...
  // event loop exec at the end of `drag_operation->exec()' if the user is fast
  // enough. Prevent this by just ignoring later drag initiations (until the
  // current drag operation ends).
...
void AbstractView::set_selection_mode(SelectionMode selection_mode) {
  ...
  if (m_selection_mode == SelectionMode::NoSelection)
    m_selection.clear();
  else if (m_selection_mode != SelectionMode::SingleSelection &&
           m_selection.size() > 1) {
...
void AbstractView::set_key_column_and_sort_order(int column,
...
void AbstractView::set_cursor(ModelIndex index,
                              SelectionUpdate selection_update,
                              bool scroll_cursor_into_view) {
  if (!model() || !index.is_valid() ||
      selection_mode() == SelectionMode::NoSelection) {
    m_cursor_index = {};
    stop_highlighted_search_timer();
    return;
  }

  if (!m_cursor_index.is_valid() ||
      model()->parent_index(m_cursor_index) != model()->parent_index(index))
    stop_highlighted_search_timer();

  if (selection_mode() == SelectionMode::SingleSelection &&
      (selection_update == SelectionUpdate::Ctrl ||
...
void AbstractView::keydown_event(KeyEvent &event) {
  ...
  if (is_searchable()) {
    ...
    } else if (event.key() != KeyCode::Key_Tab && !event.ctrl() &&
               !event.alt() && event.code_point() != 0) {
      StringBuilder sb;
...
ModelIndex AbstractView::find_next_search_match(StringView const search) {
  ...
  auto found_indices = model()->matches(search,
                                        Model::MatchesFlag::FirstMatchOnly |
                                            Model::MatchesFlag::MatchAtStart |
                                            Model::MatchesFlag::CaseInsensitive,
                                        model()->parent_index(cursor_index()));
...
void AbstractView::draw_item_text(Gfx::Painter &painter,
                                  ModelIndex const &index, bool is_selected,
                                  Gfx::IntRect const &text_rect,
                                  StringView item_text, Gfx::Font const &font,
                                  Gfx::TextAlignment alignment,
                                  Gfx::TextElision elision,
                                  size_t search_highlighting_offset) {
  if (m_edit_index == index)
    return;

  Color text_color;
  if (is_selected)
    text_color = is_focused() ? palette().selection_text()
                              : palette().inactive_selection_text();
  else
    text_color = index.data(ModelRole::ForegroundColor)
                     .to_color(palette().color(foreground_role()));
  if (index == m_highlighted_search_index) {
    ...
    painter.draw_text(
        [&](Gfx::FloatRect const &rect, Utf8CodePointIterator &) {
          if (background_searching_length > 0) {
            background_searching_length--;
            painter.fill_rect(rect.to_type<int>().inflated(0, 2),
                              palette().highlight_searching());
          }
        },
    ...
    painter.draw_text(
        [&](auto const &rect, Utf8CodePointIterator &it) {
          bool temp = false;
          if (text_searching_length > 0) {
            text_searching_length--;
            painter.draw_glyph_or_emoji(rect.location(), it, font,
                                        highlight_text_color, temp);
          } else {
    ...
    if (m_draw_item_text_with_shadow) {
      painter.draw_text(text_rect.translated(1, 1), item_text, font, alignment,
                        Color::Black, elision);
      painter.draw_text(text_rect, item_text, font, alignment, Color::White,
                        elision);
    } else {
      painter.draw_text(text_rect, item_text, font, alignment, text_color,
                        elision);
    }
  }
}
...
void AbstractView::drag_enter_event(DragEvent &event) {
  ...
  // NOTE: Right now, AbstractView accepts drags since we won't get "drag move"
  // events
  //       unless we accept the "drag enter" event.
  //       We might be able to reduce event traffic by communicating the set of
  //       drag-accepting rects in this widget to the windowing system somehow.
  event.accept();
...
void AbstractView::drag_move_event(DragEvent &event) {
  ...
  if (acceptable) {
    m_automatic_scroll_delta =
        automatic_scroll_delta_from_position(event.position());
