...
TextEditor::TextEditor(Type type) : m_type(type) {
  ...
  REGISTER_ENUM_PROPERTY("mode", mode, set_mode, Mode, {Editable, "Editable"},
                         {ReadOnly, "ReadOnly"}, {DisplayOnly, "DisplayOnly"});
...
void TextEditor::create_actions() {
  m_undo_action =
      CommonActions::make_undo_action([&](auto &) { undo(); }, this);
  m_redo_action =
      CommonActions::make_redo_action([&](auto &) { redo(); }, this);
  m_undo_action->set_enabled(false);
  m_redo_action->set_enabled(false);
  m_cut_action = CommonActions::make_cut_action([&](auto &) { cut(); }, this);
  m_copy_action =
      CommonActions::make_copy_action([&](auto &) { copy(); }, this);
  m_cut_action->set_enabled(false);
  m_copy_action->set_enabled(false);
  m_paste_action =
      CommonActions::make_paste_action([&](auto &) { paste(); }, this);
  m_paste_action->set_enabled(
      is_editable() &&
      Clipboard::the().fetch_mime_type().starts_with("text/"sv));
  if (is_multi_line()) {
    m_go_to_line_action = Action::create(
        "Go to line...", {Mod_Ctrl, Key_L},
        Gfx::Bitmap::load_from_file("/res/icons/16x16/go-to.png"sv)
            .release_value_but_fixme_should_propagate_errors(),
        [this](auto &) {
          DeprecatedString value;
          if (InputBox::show(window(), value, "Line:"sv, "Go to line"sv) ==
              InputBox::ExecResult::OK) {
  ...
  m_select_all_action = CommonActions::make_select_all_action(
      [this](auto &) { select_all(); }, this);
  m_insert_emoji_action = CommonActions::make_insert_emoji_action(
      [&](auto &) { insert_emoji(); }, this);
}
...
TextPosition TextEditor::text_position_at_content_position(
    Gfx::IntPoint content_position) const {
  ...
  case Gfx::TextAlignment::CenterLeft:
    for_each_visual_line(
        line_index,
        [&](Gfx::IntRect const &rect, auto &view, size_t start_of_line,
            [[maybe_unused]] bool is_last_visual_line) {
          if (is_multi_line() && !rect.contains_vertically(position.y()) &&
              !is_last_visual_line)
            return IterationDecision::Continue;
          column_index = start_of_line;
          if (position.x() <= 0) {
            // We're outside the text on the left side, put cursor at column 0
            // on this visual line.
          } else {
            int glyph_x = 0;
            size_t i = 0;
            for (; i < view.length(); ++i) {
              int advance = font().glyph_or_emoji_width(view.code_points()[i]) +
                            font().glyph_spacing();
  ...
  case Gfx::TextAlignment::CenterRight:
    ...
    column_index = (position.x() - content_x_for_position({line_index, 0}) +
                    fixed_glyph_width() / 2) /
                   fixed_glyph_width();
    break;
  default:
...
TextPosition TextEditor::text_position_at(Gfx::IntPoint widget_position) const {
  ...
  content_position.translate_by(horizontal_scrollbar().value(),
                                vertical_scrollbar().value());
  content_position.translate_by(
      -(m_horizontal_content_padding + ruler_width() + gutter_width()), 0);
...
void TextEditor::doubleclick_event(MouseEvent &event) {
  ...
  if (m_substitution_code_point.has_value()) {
    // NOTE: If we substitute the code points, we don't want double clicking to
    // only select a single word, since
...
void TextEditor::mousedown_event(MouseEvent &event) {
  ...
  if (m_triple_click_timer.is_valid() &&
      m_triple_click_timer.elapsed_time() < 250_ms) {
...
void TextEditor::mousemove_event(MouseEvent &event) {
  ...
  if (m_ruler_visible &&
      (ruler_rect_in_inner_coordinates().contains(event.position()))) {
...
int TextEditor::ruler_width() const {
  ...
  auto padding = 5 + (font().is_fixed_width()
                          ? 1
                          : (line_count_digits - (line_count() < 10 ? -1 : 0)));
  auto widest_numeral = font().bold_variant().glyph_width('4');
  return line_count() < 10 ? (line_count_digits + 1) * widest_numeral + padding
                           : line_count_digits * widest_numeral + padding;
}
...
Gfx::IntRect TextEditor::ruler_content_rect(size_t line_index) const {
  if (!m_ruler_visible)
    return {};
  return {0 - ruler_width() + horizontal_scrollbar().value(),
          line_content_rect(line_index).y(), ruler_width(),
...
Gfx::IntRect TextEditor::gutter_content_rect(size_t line_index) const {
  if (!m_gutter_visible)
    return {};
  return {0 - ruler_width() - gutter_width() + horizontal_scrollbar().value(),
          line_content_rect(line_index).y(), gutter_width(),
          line_content_rect(line_index).height()};
}
...
Gfx::IntRect TextEditor::visible_text_rect_in_inner_coordinates() const {
  return {m_horizontal_content_padding +
              (m_ruler_visible ? (ruler_rect_in_inner_coordinates().right() + 1)
                               : 0),
          0,
          frame_inner_rect().width() - (m_horizontal_content_padding * 2) -
              width_occupied_by_vertical_scrollbar() - ruler_width(),
          frame_inner_rect().height() -
              height_occupied_by_horizontal_scrollbar()};
}

void TextEditor::paint_event(PaintEvent &event) {
  Color widget_background_color = palette().color(
      is_enabled() ? background_role() : Gfx::ColorRole::Window);
  ...
  // TODO(ftommasi): TextEditor calls draw_text here
  //  NOTE: This lambda and TextEditor::text_width_for_font() are used to
  //  substitute all glyphs with m_substitution_code_point if necessary.
  //        Painter::draw_text() and Gfx::Font::width() should not be called
  //        directly, but using this lambda and
  //        TextEditor::text_width_for_font().
  auto draw_text = [&](Gfx::IntRect const &rect, auto const &raw_text,
                       Gfx::Font const &font, Gfx::TextAlignment alignment,
                       Gfx::TextAttributes attributes, bool substitute = true) {
    if (m_substitution_code_point.has_value() && substitute) {
      painter.draw_text(rect, substitution_code_point_view(raw_text.length()),
                        font, alignment, attributes.color);
    } else {
      painter.draw_text(rect, raw_text, font, alignment, attributes.color);
    }
    if (attributes.underline) {
      if (attributes.underline_style ==
          Gfx::TextAttributes::UnderlineStyle::Solid)
        painter.draw_line(
            rect.bottom_left().translated(0, 1),
            rect.bottom_right().translated(0, 1),
            attributes.underline_color.value_or(attributes.color));
      if (attributes.underline_style ==
          Gfx::TextAttributes::UnderlineStyle::Wavy)
        painter.draw_triangle_wave(
            rect.bottom_left().translated(0, 1),
            rect.bottom_right().translated(0, 1),
            attributes.underline_color.value_or(attributes.color), 2);
    }
  };

  if (is_displayonly() && is_focused()) {
    Gfx::IntRect display_rect{
        widget_inner_rect().x() + 1, widget_inner_rect().y() + 1,
        widget_inner_rect().width() - 2, widget_inner_rect().height() - 2};
  ...
  if (m_gutter_visible) {
    ...
    if (!m_ruler_visible)
      painter.draw_line(gutter_rect.top_right(), gutter_rect.bottom_right(),
                        palette().gutter_border());
  }

  if (m_ruler_visible) {
    auto ruler_rect = ruler_rect_in_inner_coordinates();
    painter.fill_rect(ruler_rect, palette().ruler());
    painter.draw_line(ruler_rect.top_right(), ruler_rect.bottom_right(),
  ...
  if (m_icon && horizontal_scrollbar_value > 0)
    painter.translate(
        min(icon_size() + icon_padding(), horizontal_scrollbar_value), 0);
  ...
  size_t last_visible_line =
      text_position_at(event.rect().bottom_right()).line();
  ...
  if (m_ruler_visible) {
    for (size_t i = first_visible_line; i <= last_visible_line; ++i) {
      ...
      // NOTE: Use Painter::draw_text() directly here, as we want to always draw
      // the line numbers in clear text.
      size_t const line_number =
          is_relative_line_number() && !is_current_line
              ? max(i, m_cursor.line()) - min(i, m_cursor.line())
              : i + 1;
      painter.draw_text(ruler_line_rect.shrunken(2, 0),
                        DeprecatedString::number(line_number),
                        is_current_line ? font().bold_variant() : font(),
                        Gfx::TextAlignment::CenterRight,
                        is_current_line ? palette().ruler_active_text()
                                        : palette().ruler_inactive_text());
    }
  }

  auto gutter_ruler_width = gutter_width() + ruler_width();
  Gfx::IntRect text_clip_rect{0, 0,
                              widget_inner_rect().width() - gutter_ruler_width,
                              widget_inner_rect().height()};
  text_clip_rect.translate_by(horizontal_scrollbar().value(),
                              vertical_scrollbar().value());
  ...
  if (document().has_spans()) {
    for (;;) {
      if (span_index >= document().spans().size() ||
          document().spans()[span_index].range.end().line() >=
              first_visible_line) {
        break;
      }
      ++span_index;
  ...
  for (size_t line_index = first_visible_line; line_index <= last_visible_line;
       ++line_index) {
    auto &line = this->line(line_index);

    bool physical_line_has_selection = has_selection &&
    ...
    if (physical_line_has_selection) {
      if (selection.start().line() < line_index)
        ...
        first_visual_line_with_selection =
            visual_line_containing(line_index, selection.start().column());

      if (selection.end().line() > line_index)
        last_visual_line_with_selection =
            m_line_visual_data[line_index].visual_line_breaks.size();
      else
        last_visual_line_with_selection =
            visual_line_containing(line_index, selection.end().column());
    }

    size_t selection_start_column_within_line =
        selection.start().line() == line_index ? selection.start().column() : 0;
    size_t selection_end_column_within_line =
        selection.end().line() == line_index ? selection.end().column()
                                             : line.length();

    size_t visual_line_index = 0;
    size_t last_start_of_visual_line = 0;
    Optional<size_t> multiline_trailing_space_offset{};
    for_each_visual_line(line_index, [&](Gfx::IntRect const &visual_line_rect,
                                         auto &visual_line_text,
                                         size_t start_of_visual_line,
                                         [[maybe_unused]] bool
                                             is_last_visual_line) {
      ScopeGuard update_last_start_of_visual_line{
          [&]() { last_start_of_visual_line = start_of_visual_line; }};

      if (is_focused() && is_multi_line() && line_index == m_cursor.line() &&
          is_cursor_line_highlighted()) {
        Gfx::IntRect visible_content_line_rect{
            visible_content_rect().x(), visual_line_rect.y(),
            widget_inner_rect().width() - gutter_ruler_width, line_height()};
        painter.fill_rect(visible_content_line_rect,
                          widget_background_color.darkened(0.9f));
      }
      // TODO(ftommasi) : this draws a rect over the extra emoji space so its
      // incprrect
      //  if constexpr (TEXTEDITOR_DEBUG)
      painter.draw_rect(visual_line_rect, Color::Cyan);

      if (!placeholder().is_empty() && document().is_empty() &&
          line_index == 0) {
        auto line_rect = visual_line_rect;
        line_rect.set_width(text_width_for_font(placeholder(), font()));
        draw_text(line_rect, placeholder(), font(), m_text_alignment,
                  {palette().color(Gfx::ColorRole::PlaceholderText)}, false);
      } else if (!document().has_spans()) {
        // Fast-path for plain text
        auto color = palette().color(
            is_enabled() ? foreground_role() : Gfx::ColorRole::DisabledText);
        if (is_displayonly() && is_focused())
          color = palette().color(is_enabled() ? Gfx::ColorRole::SelectionText
                                               : Gfx::ColorRole::DisabledText);
        draw_text(visual_line_rect, visual_line_text, font(), m_text_alignment,
                  {color});
      } else {
        auto unspanned_color = palette().color(
            is_enabled() ? foreground_role() : Gfx::ColorRole::DisabledText);
        if (is_displayonly() && is_focused())
          unspanned_color =
              palette().color(is_enabled() ? Gfx::ColorRole::SelectionText
                                           : Gfx::ColorRole::DisabledText);
        RefPtr<Gfx::Font> unspanned_font = this->font();

        size_t next_column = 0;
        Gfx::IntRect span_rect = {visual_line_rect.location(),
                                  {0, line_height()}};

        auto draw_text_helper = [&](size_t start, size_t end,
                                    RefPtr<Gfx::Font> &font,
                                    Gfx::TextAttributes text_attributes) {
          ...
          if (text_attributes.background_color.has_value()) {
            painter.fill_rect(span_rect,
                              text_attributes.background_color.value());
          }
        ...
        for (;;) {
          ...
          if (span.range.end().line() < line_index) {
            dbgln_if(TEXTEDITOR_DEBUG,
                     "spans not sorted (span end {}:{} is before current line "
                     "{}) => ignoring",
          ...
          if (span.range.start().line() > line_index ||
              (span.range.start().line() == line_index &&
               span.range.start().column() >=
                   start_of_visual_line + visual_line_text.length())) {
            // no more spans in this line, moving on
            break;
          }
          if (span.range.start().line() == span.range.end().line() &&
              span.range.end().column() < span.range.start().column()) {
            dbgln_if(TEXTEDITOR_DEBUG,
                     "span from {}:{} to {}:{} has negative length => ignoring",
          ...
          if (span.range.end().line() == line_index &&
              span.range.end().column() < start_of_visual_line + next_column) {
            dbgln_if(TEXTEDITOR_DEBUG,
                     "spans not sorted (span end {}:{} is before current "
                     "position {}:{}) => ignoring",
                     span.range.end().line(), span.range.end().column(),
                     line_index, start_of_visual_line + next_column);
            ++span_index;
            continue;
          }
          size_t span_start;
          if (span.range.start().line() < line_index ||
              span.range.start().column() < start_of_visual_line) {
            span_start = 0;
          } else {
          ...
          if (span_start < next_column) {
            dbgln_if(TEXTEDITOR_DEBUG,
                     "span started before the current position, maybe two "
                     "spans overlap? (span start {} is before current position "
                     "{}) => ignoring",
                     span_start, next_column);
            ++span_index;
            continue;
          }
          size_t span_end;
          ...
          if (span_start != next_column) {
            ...
            draw_text_helper(next_column, span_start, unspanned_font,
                             {unspanned_color});
          }
          auto font = unspanned_font;
          if (span.attributes.bold) {
            if (auto bold_font = Gfx::FontDatabase::the().get(
                    font->family(), font->presentation_size(), 700,
                    font->width(), 0))
              font = bold_font;
          }
        ...
        if (next_column < visual_line_text.length()) {
          draw_text_helper(next_column, visual_line_text.length(),
                           unspanned_font, {unspanned_color});
        }
      ...
      if (m_visualize_trailing_whitespace && line.ends_in_whitespace()) {
        ...
        if (last_non_whitespace_column.has_value())
          ...
          physical_column = 0;
        size_t end_of_visual_line =
            (start_of_visual_line + visual_line_text.length());
        if (physical_column < end_of_visual_line) {
          physical_column -= multiline_trailing_space_offset.value_or(0);

          size_t visual_column = physical_column > start_of_visual_line
                                     ? (physical_column - start_of_visual_line)
                                     : 0;
          ...
          Gfx::IntRect whitespace_rect{
              ...
              visual_line_rect.y(),
              text_width_for_font(
                  visual_line_text.substring_view(
                      visual_column, visual_line_text.length() - visual_column),
                  font()),
              visual_line_rect.height()};
          painter.fill_rect_with_dither_pattern(whitespace_rect, Color(),
                                                Color(255, 192, 192));

          if (!multiline_trailing_space_offset.has_value())
            multiline_trailing_space_offset =
                physical_column - last_start_of_visual_line;
        }
      }
      if (m_visualize_leading_whitespace && line.leading_spaces() > 0) {
        ...
        if (start_of_visual_line < physical_column) {
          size_t end_of_leading_whitespace =
              min(physical_column - start_of_visual_line,
                  visual_line_text.length());
          Gfx::IntRect whitespace_rect{
              content_x_for_position({line_index, start_of_visual_line}),
              visual_line_rect.y(),
              text_width_for_font(
                  visual_line_text.substring_view(0, end_of_leading_whitespace),
                  font()),
              visual_line_rect.height()};
          painter.fill_rect_with_dither_pattern(whitespace_rect, Color(),
                                                Color(192, 255, 192));
      ...
      if (physical_line_has_selection && window()->focused_widget() == this) {
        size_t const start_of_selection_within_visual_line =
            (size_t)max(0, (int)selection_start_column_within_line -
                               (int)start_of_visual_line);
        size_t const end_of_selection_within_visual_line =
            min(selection_end_column_within_line - start_of_visual_line,
                visual_line_text.length());

        bool current_visual_line_has_selection =
            start_of_selection_within_visual_line !=
                end_of_selection_within_visual_line &&
            ((line_index != selection.start().line() &&
              line_index != selection.end().line()) ||
             (visual_line_index >= first_visual_line_with_selection &&
              visual_line_index <= last_visual_line_with_selection));
        if (current_visual_line_has_selection) {
          bool selection_begins_on_current_visual_line =
              visual_line_index == first_visual_line_with_selection;
          bool selection_ends_on_current_visual_line =
              visual_line_index == last_visual_line_with_selection;

          int selection_left =
              selection_begins_on_current_visual_line
                  ? content_x_for_position(
                        {line_index,
                         (size_t)selection_start_column_within_line})
                  : m_horizontal_content_padding;

          int selection_right =
              selection_ends_on_current_visual_line
                  ? content_x_for_position(
                        {line_index, (size_t)selection_end_column_within_line})
                  : visual_line_rect.right() + 1;

          Gfx::IntRect selection_rect{selection_left, visual_line_rect.y(),
                                      selection_right - selection_left,
                                      visual_line_rect.height()};

          Color background_color = is_focused()
                                       ? palette().selection()
                                       : palette().inactive_selection();
          Color text_color = is_focused() ? palette().selection_text()
                                          : palette().inactive_selection_text();

          painter.fill_rect(selection_rect, background_color);

          if (visual_line_text.code_points()) {
            Utf32View visual_selected_text{
                visual_line_text.code_points() +
                    start_of_selection_within_visual_line,
                end_of_selection_within_visual_line -
                    start_of_selection_within_visual_line};

            draw_text(selection_rect, visual_selected_text, font(),
                      Gfx::TextAlignment::CenterLeft, {text_color});
          }
        }
      }
  ...
  if (is_enabled() && is_focused() && !focus_preempted() && m_cursor_state &&
      !is_displayonly())
...
void TextEditor::select_all() {
  ...
  TextPosition end_of_document{line_count() - 1,
...
void TextEditor::keydown_event(KeyEvent &event) {
  ...
  if (m_autocomplete_box && m_autocomplete_box->is_visible() &&
      (event.key() == KeyCode::Key_Return || event.key() == KeyCode::Key_Tab)) {
    TemporaryChange change{m_should_keep_autocomplete_box, true};
    if (m_autocomplete_box->apply_suggestion() ==
        CodeComprehension::AutocompleteResultEntry::
            HideAutocompleteAfterApplying::Yes)
      hide_autocomplete();
    else
  ...
  if (m_autocomplete_box && m_autocomplete_box->is_visible() &&
      event.key() == KeyCode::Key_Up) {
    m_autocomplete_box->previous_suggestion();
    return;
  }

  if (m_autocomplete_box && m_autocomplete_box->is_visible() &&
      event.key() == KeyCode::Key_Down) {
  ...
  ArmedScopeGuard update_autocomplete{[&] { try_update_autocomplete(); }};

  if (is_multi_line() && !event.alt() && event.ctrl() &&
      event.key() == KeyCode::Key_Return) {
    if (!is_editable())
      return;

    size_t indent_length = current_line().leading_spaces();
    DeprecatedString indent =
        current_line().to_utf8().substring(0, indent_length);
    auto insert_pos = event.shift() ? InsertLineCommand::InsertPosition::Above
                                    : InsertLineCommand::InsertPosition::Below;
    execute<InsertLineCommand>(m_cursor, move(indent), insert_pos);
    return;
  }

  if (is_multi_line() && !event.shift() && !event.alt() && event.ctrl() &&
      event.key() == KeyCode::Key_Space) {
  ...
  if (event.key() == KeyCode::Key_Delete) {
    ...
    if (m_cursor.column() < current_line().length()) {
      ...
      TextRange erased_range(
          m_cursor, {m_cursor.line(), m_cursor.column() + erase_count});
      execute<RemoveTextCommand>(document().text_in_range(erased_range),
                                 erased_range);
      return;
    }
    if (m_cursor.column() == current_line().length() &&
      ...
      if (event.modifiers() == Mod_Ctrl) {
        erase_count = document()
                          .first_word_break_after({m_cursor.line() + 1, 0})
                          .column();
      }
  ...
  if (event.key() == KeyCode::Key_Backspace) {
    ...
    if (m_cursor.column() > 0) {
      ...
      if (event.modifiers() == Mod_Ctrl) {
        auto word_break_pos =
            document().first_word_break_before(m_cursor, true);
        erase_count = m_cursor.column() - word_break_pos.column();
      } else if (current_line().first_non_whitespace_column() >=
                 m_cursor.column()) {
        int new_column;
        if (m_cursor.column() % m_soft_tab_width == 0)
          ...
          new_column =
              (m_cursor.column() / m_soft_tab_width) * m_soft_tab_width;
        erase_count = m_cursor.column() - new_column;
      }

      // Backspace within line
      TextRange erased_range({m_cursor.line(), m_cursor.column() - erase_count},
                             m_cursor);
  ...
  if ((!event.ctrl() || event.altgr()) && !event.alt() &&
      event.code_point() != 0) {
    TemporaryChange change{m_should_keep_autocomplete_box, true};
  ...
  if (event.ctrl() && event.key() == KeyCode::Key_Slash) {
    if (m_highlighter != nullptr) {
      ...
      auto range = has_selection()
                       ? selection().normalized()
                       : TextRange{{m_cursor.line(), m_cursor.column()},
...
bool TextEditor::is_indenting_selection() {
  auto const selection_start = m_selection.start() > m_selection.end()
                                   ? m_selection.end()
                                   : m_selection.start();
  auto const selection_end = m_selection.end() > m_selection.start()
                                 ? m_selection.end()
                                 : m_selection.start();
  auto const whole_line_selected =
      selection_end.column() - selection_start.column() >=
      current_line().length() - current_line().first_non_whitespace_column();
...
void TextEditor::indent_selection() {
  ...
  if (is_indenting_selection()) {
    execute<IndentSelection>(m_soft_tab_width,
                             TextRange(selection_start, selection_end));
    m_selection.set_start(
        {selection_start.line(), selection_start.column() + m_soft_tab_width});
    m_selection.set_end(
        {selection_end.line(), selection_end.column() + m_soft_tab_width});
...
void TextEditor::unindent_selection() {
  ...
  if (current_line().first_non_whitespace_column() != 0) {
    if (current_line().first_non_whitespace_column() > m_soft_tab_width &&
        selection_start.column() != 0) {
      m_selection.set_start({selection_start.line(),
                             selection_start.column() - m_soft_tab_width});
      m_selection.set_end(
          {selection_end.line(), selection_end.column() - m_soft_tab_width});
    } else if (selection_start.column() != 0) {
      m_selection.set_start(
          {selection_start.line(),
           selection_start.column() - current_line().leading_spaces()});
      m_selection.set_end(
          {selection_end.line(),
           selection_end.column() - current_line().leading_spaces()});
    }
    execute<UnindentSelection>(m_soft_tab_width,
                               TextRange(selection_start, selection_end));
  }
}
...
void TextEditor::unindent_line() {
  if (current_line().first_non_whitespace_column() != 0) {
    auto const unindent_size =
        current_line().leading_spaces() < m_soft_tab_width
            ? current_line().leading_spaces()
            : m_soft_tab_width;
    auto const temp_column = m_cursor.column();

    execute<UnindentSelection>(
        unindent_size,
        TextRange({m_cursor.line(), 0},
                  {m_cursor.line(), line(m_cursor.line()).length()}));

    set_cursor({m_cursor.line(), temp_column <= unindent_size
                                     ? 0
                                     : temp_column - unindent_size});
  }
}
...
void TextEditor::delete_from_line_start_to_cursor() {
  TextPosition start(m_cursor.line(),
                     current_line().first_non_whitespace_column());
...
int TextEditor::content_x_for_position(TextPosition const &position) const {
  ...
  switch (m_text_alignment) {
  case Gfx::TextAlignment::CenterLeft:
    for_each_visual_line(position.line(), [&](Gfx::IntRect const &,
                                              auto &visual_line_view,
                                              size_t start_of_visual_line,
                                              bool is_last_visual_line) {
      size_t offset_in_visual_line = position.column() - start_of_visual_line;
      auto before_line_end =
          is_last_visual_line
              ? (offset_in_visual_line <= visual_line_view.length())
              : (offset_in_visual_line < visual_line_view.length());
      if (position.column() >= start_of_visual_line && before_line_end) {
        if (offset_in_visual_line == 0) {
          x_offset = 0;
        } else {
          x_offset = text_width_for_font(
              visual_line_view.substring_view(0, offset_in_visual_line),
              font());
    ...
    return m_horizontal_content_padding +
           ((is_single_line() && icon()) ? (icon_size() + icon_padding()) : 0) +
           x_offset;
  case Gfx::TextAlignment::CenterRight:
    // FIXME
    VERIFY(!is_wrapping_enabled());
    return content_width() - m_horizontal_content_padding -
           (line.length() * fixed_glyph_width()) +
           (position.column() * fixed_glyph_width());
  default:
    VERIFY_NOT_REACHED();
  }
}

int TextEditor::text_width_for_font(auto const &text,
...
Gfx::IntRect
TextEditor::content_rect_for_position(TextPosition const &position) const {
  ...
  for_each_visual_line(
      position.line(),
      [&](Gfx::IntRect const &visual_line_rect, auto &view,
          size_t start_of_visual_line, bool is_last_visual_line) {
        auto before_line_end =
            is_last_visual_line
                ? ((position.column() - start_of_visual_line) <= view.length())
                : ((position.column() - start_of_visual_line) < view.length());
        if (position.column() >= start_of_visual_line && before_line_end) {
          // NOTE: We have to subtract the horizontal padding here since it's
          // part of the visual line rect
          //       *and* included in what we get from content_x_for_position().
          rect = {visual_line_rect.x() + x - (m_horizontal_content_padding),
                  visual_line_rect.y(),
...
void TextEditor::scroll_position_into_view(TextPosition const &position) {
  ...
  if (position.column() == 0)
    ...
    rect.set_x(content_x_for_position(
                   {position.line(), line(position.line()).length()}) +
               2);
...
Gfx::IntRect TextEditor::line_content_rect(size_t line_index) const {
  auto &line = this->line(line_index);
  if (is_single_line()) {
    Gfx::IntRect line_rect = {content_x_for_position({line_index, 0}), 0,
  ...
  return {content_x_for_position({line_index, 0}),
          (int)line_index * line_height(),
          text_width_for_font(line.view(), font()), line_height()};
}
...
void TextEditor::set_cursor_and_focus_line(size_t line, size_t column) {
  ...
  if (line > 1 && line < index_max) {
    ...
    Gfx::IntRect rect = {0, line_content_rect(line).y(), 1,
                         frame_inner_rect().height()};
    scroll_into_view(rect, false, true);
  }
}

void TextEditor::update_cursor() { update(line_widget_rect(m_cursor.line())); }
...
void TextEditor::set_cursor(TextPosition const &a_position) {
  ...
  if (m_cursor != position && is_visual_data_up_to_date()) {
    // NOTE: If the old cursor is no longer valid, repaint everything just in
    // case.
...
DeprecatedString TextEditor::text() const { return document().text(); }
...
void TextEditor::delete_text_range(TextRange range) {
  ...
  execute<RemoveTextCommand>(document().text_in_range(normalized_range),
                             normalized_range);
  did_change();
...
void TextEditor::insert_at_cursor_or_replace_selection(StringView text) {
  ...
  auto const should_clear_last_line =
      text == "\n" && clear_length > 0 &&
...
void TextEditor::replace_all_text_without_resetting_undo_stack(
    StringView text) {
...
void TextEditor::defer_reflow() { ++m_reflow_deferred; }
...
void TextEditor::try_show_autocomplete(
  ...
  force_update_autocomplete(
      [&, user_requested_autocomplete = move(user_requested_autocomplete)] {
        if (user_requested_autocomplete == Yes ||
            m_autocomplete_box->has_suggestions()) {
          auto position = content_rect_for_position(cursor())
                              .translated(0, -visible_content_rect().y())
                              .bottom_right()
                              .translated(screen_relative_rect()
                                              .top_left()
                                              .translated(ruler_width(), 0)
                                              .translated(10, 5));
...
void TextEditor::force_update_autocomplete(Function<void()> callback) {
  if (m_autocomplete_provider) {
    m_autocomplete_provider->provide_completions(
        [&, callback = move(callback)](auto completions) {
...
void TextEditor::context_menu_event(ContextMenuEvent &event) {
  ...
  m_insert_emoji_action->set_enabled(on_emoji_input &&
                                     !window()->blocks_emoji_input());
...
size_t TextEditor::visual_line_containing(size_t line_index,
                                          size_t column) const {
  size_t visual_line_index = 0;
  for_each_visual_line(line_index,
                       [&](Gfx::IntRect const &, auto &view,
                           size_t start_of_visual_line,
                           [[maybe_unused]] bool is_last_visual_line) {
                         if (column >= start_of_visual_line &&
                             ((column - start_of_visual_line) < view.length()))
                           return IterationDecision::Break;
                         ++visual_line_index;
                         return IterationDecision::Continue;
                       });
...
void TextEditor::recompute_visual_lines(size_t line_index) {
  ...
  if (is_wrapping_enabled()) {
    ...
    for (size_t i = 0; i < line.length(); ++i) {
      ...
      if ((line_width_so_far + glyph_width + glyph_spacing) > available_width) {
        if (m_wrapping_mode == WrappingMode::WrapAtWords &&
  ...
  if (is_wrapping_enabled())
    visual_data.visual_rect = {
        m_horizontal_content_padding, 0, available_width,
        static_cast<int>(visual_data.visual_line_breaks.size()) *
            line_height()};
  else
    visual_data.visual_rect = {m_horizontal_content_padding, 0,
                               text_width_for_font(line.view(), font()),
                               line_height()};
}

template <typename Callback>
void TextEditor::for_each_visual_line(size_t line_index,
  ...
  for (auto visual_line_break : visual_data.visual_line_breaks) {
    auto visual_line_view = Utf32View(line.code_points() + start_of_line,
                                      visual_line_break - start_of_line);
    ...
    if (is_right_text_alignment(text_alignment()))
      visual_line_rect.set_right_without_resize(
          editor_visible_text_rect.right());
    if (is_single_line()) {
...
void TextEditor::document_did_update_undo_stack() {
  ...
  m_undo_action->set_text(
      make_action_text("&Undo"sv, document().undo_stack().undo_action_text()));
  m_redo_action->set_text(
      make_action_text("&Redo"sv, document().undo_stack().redo_action_text()));

  // FIXME: This is currently firing more often than it should.
  //        Ideally we'd only send this out when the undo stack modified state
  //        actually changes.
...
void TextEditor::cursor_did_change() { hide_autocomplete_if_needed(); }

void TextEditor::clipboard_content_did_change(
    DeprecatedString const &mime_type) {
  m_paste_action->set_enabled(is_editable() &&
                              mime_type.starts_with("text/"sv));
}
...
void TextEditor::set_syntax_highlighter(
    OwnPtr<Syntax::Highlighter> highlighter) {
  if (m_highlighter)
...
void TextEditor::set_autocomplete_provider(
    OwnPtr<AutocompleteProvider> &&provider) {
  if (m_autocomplete_provider)
...
void TextEditor::set_should_autocomplete_automatically(bool value) {
  ...
  if (value) {
    ...
    m_autocomplete_timer =
        Core::Timer::create_single_shot(
            m_automatic_autocomplete_delay_ms,
            [this] {
              if (m_autocomplete_box && !m_autocomplete_box->is_visible())
...
TextRange TextEditor::find_text(StringView needle, SearchDirection direction,
                                GUI::TextDocument::SearchShouldWrap should_wrap,
                                bool use_regex, bool match_case) {
  GUI::TextRange range{};
  if (direction == SearchDirection::Forward) {
    range = document().find_next(
        needle,
        m_search_result_index.has_value()
            ? m_search_results[*m_search_result_index].end()
            : GUI::TextPosition{},
        should_wrap, use_regex, match_case);
  } else {
    range = document().find_previous(
        needle,
        m_search_result_index.has_value()
            ? m_search_results[*m_search_result_index].start()
            : GUI::TextPosition{},
        should_wrap, use_regex, match_case);
  }
...
void TextEditor::on_search_results(GUI::TextRange current,
                                   Vector<GUI::TextRange> all_results) {
  m_search_result_index.clear();
  m_search_results.clear();
  ...
  for (size_t i = 0; i < m_search_results.size(); ++i) {
    ...
    span.attributes.color = Color::from_argb(
        0xff000000); // So text without spans from a highlighter will have color
