...
Painter::Painter(Gfx::Bitmap &bitmap) : m_target(bitmap) {
  int scale = bitmap.scale();
  VERIFY(bitmap.format() == Gfx::BitmapFormat::BGRx8888 ||
         bitmap.format() == Gfx::BitmapFormat::BGRA8888);
  ...
  paint_style.paint(a_rect, [&](PaintStyle::SamplerFunction sample) {
    for (int y = 0; y < clipped_rect.height(); ++y) {
      for (int x = 0; x < clipped_rect.width(); ++x) {
        IntPoint point(x, y);
        set_physical_pixel(point + clipped_rect.location(),
                           sample(point + start_offset), true);
      }
    }
  });
}

void Painter::fill_rect_with_dither_pattern(IntRect const &a_rect,
...
void Painter::fill_rect_with_checkerboard(IntRect const &a_rect,
                                          IntSize cell_size, Color color_dark,
                                          Color color_light) {
  VERIFY(scale() == 1); // FIXME: Add scaling support.
  ...
  int prologue_length =
      min(rect.width(), cell_size.width() - ((rect.x() - translated_rect.x()) %
                                             cell_size.width()));
  int number_of_aligned_strips =
      (rect.width() - prologue_length) / cell_size.width();
  ...
  for (int i = 0; i < rect.height(); ++i) {
...

    {
      ...
      auto color =
          (odd_row ^ odd_cell) ? color_light.value() : color_dark.value();
...
void Painter::fill_rect_with_gradient(Orientation orientation,
                                      IntRect const &a_rect,
  ...
  return fill_rect_with_linear_gradient(
      a_rect, Array{ColorStop{gradient_start, 0}, ColorStop{gradient_end, 1}},
      orientation == Orientation::Horizontal ? 90.0f : 0.0f);
}

void Painter::fill_rect_with_gradient(IntRect const &a_rect,
                                      Color gradient_start,
                                      Color gradient_end) {
  return fill_rect_with_gradient(Orientation::Horizontal, a_rect,
                                 gradient_start, gradient_end);
}

void Painter::fill_rect_with_rounded_corners(IntRect const &a_rect, Color color,
                                             int radius) {
  return fill_rect_with_rounded_corners(a_rect, color, radius, radius, radius,
                                        radius);
}
...
void Painter::fill_rect_with_rounded_corners(IntRect const &a_rect, Color color,
  ...
  if (!top_left_radius && !top_right_radius && !bottom_right_radius &&
      !bottom_left_radius)
  ...
  IntRect top_left_corner = {a_rect.x(), a_rect.y(), top_left_radius,
                             top_left_radius};
  IntRect top_right_corner = {a_rect.x() + a_rect.width() - top_right_radius,
                              a_rect.y(), top_right_radius, top_right_radius};
  IntRect bottom_right_corner = {
      a_rect.x() + a_rect.width() - bottom_right_radius,
      a_rect.y() + a_rect.height() - bottom_right_radius, bottom_right_radius,
      bottom_right_radius};
  IntRect bottom_left_corner = {
      a_rect.x(), a_rect.y() + a_rect.height() - bottom_left_radius,
      bottom_left_radius, bottom_left_radius};

  IntRect top_rect = {a_rect.x() + top_left_radius, a_rect.y(),
                      a_rect.width() - top_left_radius - top_right_radius,
                      top_left_radius};
  IntRect right_rect = {a_rect.x() + a_rect.width() - top_right_radius,
                        a_rect.y() + top_right_radius, top_right_radius,
                        a_rect.height() - top_right_radius -
                            bottom_right_radius};
  IntRect bottom_rect = {a_rect.x() + bottom_left_radius,
                         a_rect.y() + a_rect.height() - bottom_right_radius,
                         a_rect.width() - bottom_left_radius -
                             bottom_right_radius,
                         bottom_right_radius};
  IntRect left_rect = {a_rect.x(), a_rect.y() + top_left_radius,
                       bottom_left_radius,
                       a_rect.height() - top_left_radius - bottom_left_radius};

  IntRect inner = {left_rect.x() + left_rect.width(), left_rect.y(),
  ...
  if (top_left_radius)
    fill_rounded_corner(top_left_corner, top_left_radius, color,
                        CornerOrientation::TopLeft);
  if (top_right_radius)
    fill_rounded_corner(top_right_corner, top_right_radius, color,
                        CornerOrientation::TopRight);
  if (bottom_left_radius)
    fill_rounded_corner(bottom_left_corner, bottom_left_radius, color,
                        CornerOrientation::BottomLeft);
  if (bottom_right_radius)
    fill_rounded_corner(bottom_right_corner, bottom_right_radius, color,
                        CornerOrientation::BottomRight);
}

void Painter::fill_rounded_corner(IntRect const &a_rect, int radius,
                                  Color color, CornerOrientation orientation) {
  // Care about clipping
  ...
  auto is_in_circle = [&](int x, int y) {
    int distance2 = (circle_center.x() - x) * (circle_center.x() - x) +
                    (circle_center.y() - y) * (circle_center.y() - y);
    // To reflect the grid and be compatible with the
    // draw_circle_arc_intersecting algorithm add 1/2 to the radius
...
void Painter::draw_circle_arc_intersecting(IntRect const &a_rect,
                                           IntPoint center, int radius,
  ...
  auto is_on_arc = [&](int x, int y) {
    int distance2 = (center.x() - x) * (center.x() - x) +
                    (center.y() - y) * (center.y() - y);
    // Is within a circle of radius 1/2 around (x,y), so basically within the
    // current pixel. Technically this is angle-dependent and should be between
    // 1/2 and sqrt(2)/2, but this works.
    return distance2 <= (radius2 + radius + 0.25) &&
           distance2 >= (radius2 - radius + 0.25);
  };
  ...
  return draw_circle_arc_intersecting(a_rect, center, radius - 1, color,
                                      thickness - 1);
}

// The callback will only be called for a quarter of the ellipse, the user is
// intended to deduce other points. As the coordinate space is relative to the
// center of the rectangle, it's simply (x, y), (x, -y), (-x, y) and (-x, -y).
static void on_each_ellipse_point(IntRect const &rect,
                                  Function<void(IntPoint)> &&callback) {
  ...
  // For region 2:
  decision_parameter = b_square * ((x + 0.5) * (x + 0.5)) +
                       a_square * ((y - 1) * (y - 1)) - a_square * b_square;
...
void Painter::fill_ellipse(IntRect const &a_rect, Color color) {
  ...
  on_each_ellipse_point(rect, [this, &color, center](IntPoint position) {
    IntPoint const directions[4] = {{position.x(), position.y()},
...
void Painter::draw_ellipse_intersecting(IntRect const &rect, Color color,
                                        int thickness) {
  VERIFY(scale() == 1); // FIXME: Add scaling support.
  ...
  on_each_ellipse_point(
      rect, [this, &color, thickness, center](IntPoint position) {
        IntPoint const directions[4] = {{position.x(), position.y()},
                                        {position.x(), -position.y()},
                                        {-position.x(), position.y()},
                                        {-position.x(), -position.y()}};
...
static void for_each_pixel_around_rect_clockwise(RectType const &rect,
                                                 Callback callback) {
  if (rect.is_empty())
    return;
...
void Painter::draw_rect(IntRect const &a_rect, Color color, bool rough) {
  ...
  if (rect.top() >= clipped_rect.top() && rect.top() <= clipped_rect.bottom()) {
    int start_x =
        rough ? max(rect.x() + 1, clipped_rect.x()) : clipped_rect.x();
    int width = rough ? min(rect.width() - 2, clipped_rect.width())
                      : clipped_rect.width();
    for (int i = 0; i < scale; ++i)
      fill_physical_scanline_with_draw_op(
          rect.top() * scale + i, start_x * scale, width * scale, color);
    ++min_y;
  }
  if (rect.bottom() >= clipped_rect.top() &&
    ...
    int width = rough ? min(rect.width() - 2, clipped_rect.width())
                      : clipped_rect.width();
    for (int i = 0; i < scale; ++i)
      fill_physical_scanline_with_draw_op(max_y * scale + i, start_x * scale,
                                          width * scale, color);
    --max_y;
  }
  ...
  if (draw_left_side && draw_right_side) {
    ...
    for (int y = min_y * scale; y <= max_y * scale; ++y) {
      ...
      if (draw_right_side)
        for (int i = 0; i < scale; ++i)
          set_physical_pixel_with_draw_op(bits[rect.right() * scale + i],
                                          color);
    }
  }
}

void Painter::draw_rect_with_thickness(IntRect const &rect, Color color,
                                       int thickness) {
  if (thickness <= 0)
    return;
  ...
  IntPoint p3 = {rect.location().x() + rect.width(),
...
void Painter::draw_bitmap(IntPoint p, CharacterBitmap const &bitmap,
                          Color color) {
  VERIFY(scale() == 1); // FIXME: Add scaling support.
  ...
  size_t const dst_skip = m_target->pitch() / sizeof(ARGB32);
  char const *bitmap_row =
      &bitmap.bits()[first_row * bitmap.width() + first_column];
...
void Painter::draw_bitmap(IntPoint p, GlyphBitmap const &bitmap, Color color) {
  ...
  int scale = this->scale();
  ARGB32 *dst =
      m_target->scanline(clipped_rect.y() * scale) + clipped_rect.x() * scale;
  size_t const dst_skip = m_target->pitch() / sizeof(ARGB32);
        ...
        if (bitmap.bit_at((j + first_column), row)) {
          for (int iy = 0; iy < scale; ++iy)
            for (int ix = 0; ix < scale; ++ix) {
              ...
              dst[pixel_index] =
                  Color::from_argb(dst[pixel_index]).blend(color).value();
            }
        }
      }
      dst += dst_skip * scale;
...
void Painter::draw_triangle(IntPoint offset,
                            ReadonlySpan<IntPoint> control_points,
                            Color color) {
  VERIFY(control_points.size() == 3);
  draw_triangle(control_points[0] + offset, control_points[1] + offset,
                control_points[2] + offset, color);
}
...
void Painter::draw_triangle(IntPoint a, IntPoint b, IntPoint c, Color color) {
  ...
  for (int y = max(p0.y(), clip.top()); y <= min(p2.y(), clip.bottom()); y++) {
    Optional<int> x0 = l0.intersection_on_x(y), x1 = l1.intersection_on_x(y),
    ...
    for (int x = max(left_bound, clip.left());
         x <= min(right_bound, clip.right()); x++) {
      scanline[x] = rgba;
    }
  }
}
...
//        Ideally we'd have a more generic solution that allows any source
//        format.
static void swap_red_and_blue_channels(Color &color) {
  u32 rgba = color.value();
  u32 bgra = (rgba & 0xff00ff00) | ((rgba & 0x000000ff) << 16) |
             ((rgba & 0x00ff0000) >> 16);
...
static void do_blit_with_opacity(BlitState &state) {
  for (int row = 0; row < state.row_count; ++row) {
    for (int x = 0; x < state.column_count; ++x) {
      Color dest_color = (has_alpha & BlitState::DstAlpha)
                             ? Color::from_argb(state.dst[x])
                             : Color::from_rgb(state.dst[x]);
...
void Painter::blit_with_opacity(IntPoint position, Gfx::Bitmap const &source,
                                IntRect const &a_src_rect, float opacity,
                                bool apply_alpha) {
  VERIFY(scale() >= source.scale() &&
         "painter doesn't support downsampling scale factors");
  ...
  if (scale() != source.scale())
    return draw_scaled_bitmap({position, safe_src_rect.size()}, source,
                              safe_src_rect, opacity);

  auto dst_rect =
      IntRect(position, safe_src_rect.size()).translated(translation());
  auto clipped_rect = dst_rect.intersected(clip_rect());
  if (clipped_rect.is_empty())
    return;
  ...
  BlitState blit_state{
      .src = source.scanline(src_rect.top() + first_row) + src_rect.left() +
             first_column,
...
void Painter::blit_filtered(IntPoint position, Gfx::Bitmap const &source,
                            IntRect const &src_rect,
                            Function<Color(Color)> filter) {
  VERIFY((source.scale() == 1 || source.scale() == scale()) &&
         "blit_filtered only supports integer upsampling");
  ...
  if (s == 1) {
    ARGB32 const *src = source.scanline(safe_src_rect.top() + first_row) +
                        safe_src_rect.left() + first_column;
    ...
    for (int row = first_row; row <= last_row; ++row) {
      for (int x = 0; x <= (last_column - first_column); ++x) {
        ...
        if (alpha == 0xff) {
          ...
          dst[x] = Color::from_argb(dst[x])
                       .blend(filter(Color::from_argb(src[x])))
                       .value();
      }
      dst += dst_skip;
      src += src_skip;
    ...
    for (int row = first_row; row <= last_row; ++row) {
      ARGB32 const *src = source.scanline(safe_src_rect.top() + row / s) +
                          safe_src_rect.left() + first_column / s;
      for (int x = 0; x <= (last_column - first_column); ++x) {
        ...
        if (alpha == 0xff) {
          ...
          dst[x] = Color::from_argb(dst[x])
                       .blend(filter(Color::from_argb(src[x / s])))
                       .value();
      }
      dst += dst_skip;
    }
...
void Painter::blit_brightened(IntPoint position, Gfx::Bitmap const &source,
                              IntRect const &src_rect) {
  return blit_filtered(position, source, src_rect,
                       [](Color src) { return src.lightened(); });
}

void Painter::blit_dimmed(IntPoint position, Gfx::Bitmap const &source,
                          IntRect const &src_rect) {
...
void Painter::draw_tiled_bitmap(IntRect const &a_dst_rect,
                                Gfx::Bitmap const &source) {
  VERIFY((source.scale() == 1 || source.scale() == scale()) &&
         "draw_tiled_bitmap only supports integer upsampling");
  ...
  if (source.format() == BitmapFormat::BGRx8888 ||
      source.format() == BitmapFormat::BGRA8888) {
    int s = scale / source.scale();
    if (s == 1) {
      int x_start = first_column + a_dst_rect.left() * scale;
      for (int row = first_row; row <= last_row; ++row) {
        ARGB32 const *sl = source.scanline((row + a_dst_rect.top() * scale) %
      ...
      int x_start = first_column + a_dst_rect.left() * scale;
      for (int row = first_row; row <= last_row; ++row) {
        ARGB32 const *sl = source.scanline(
            ((row + a_dst_rect.top() * scale) / s) % source.physical_height());
        for (int x = x_start; x < clipped_rect.width() + x_start; ++x) {
...
void Painter::blit_offset(IntPoint a_position, Gfx::Bitmap const &source,
                          IntRect const &a_src_rect, IntPoint offset) {
...
void Painter::blit(IntPoint position, Gfx::Bitmap const &source,
                   IntRect const &a_src_rect, float opacity, bool apply_alpha) {
  VERIFY(scale() >= source.scale() &&
         "painter doesn't support downsampling scale factors");
  ...
  // If we get here, the Painter might have a scale factor, but the source
  // bitmap has the same scale factor. We need to transform from logical to
  // physical coordinates, but we can just copy pixels without resampling.
  auto dst_rect =
      IntRect(position, safe_src_rect.size()).translated(translation());
  auto clipped_rect = dst_rect.intersected(clip_rect());
  if (clipped_rect.is_empty())
  ...
  if (source.format() == BitmapFormat::BGRx8888 ||
      source.format() == BitmapFormat::BGRA8888) {
    ARGB32 const *src = source.scanline(src_rect.top() + first_row) +
  ...
  if (source.format() == BitmapFormat::RGBA8888) {
    u32 const *src = source.scanline(src_rect.top() + first_row) +
                     src_rect.left() + first_column;
  ...
  if (Bitmap::is_indexed(source.format())) {
    u8 const *src = source.scanline_u8(src_rect.top() + first_row) +
                    src_rect.left() + first_column;
...
do_draw_integer_scaled_bitmap(Gfx::Bitmap &target, IntRect const &dst_rect,
                              IntRect const &src_rect,
                              Gfx::Bitmap const &source, int hfactor,
                              int vfactor, GetPixel get_pixel, float opacity) {
  ...
  for (int y = 0; y < src_rect.height(); ++y) {
    ...
    for (int x = 0; x < src_rect.width(); ++x) {
      auto src_pixel =
          get_pixel(source, x + src_rect.left(), y + src_rect.top());
      if (has_opacity)
...
template <bool has_alpha_channel, Painter::ScalingMode scaling_mode,
          typename GetPixel>
ALWAYS_INLINE static void
do_draw_scaled_bitmap(Gfx::Bitmap &target, IntRect const &dst_rect,
                      IntRect const &clipped_rect, Gfx::Bitmap const &source,
                      FloatRect const &src_rect, GetPixel get_pixel,
                      float opacity) {
  ...
  if (scaling_mode == Painter::ScalingMode::NearestFractional) {
    int hfactor =
        (dst_rect.width() + int_src_rect.width() - 1) / int_src_rect.width();
    int vfactor =
        (dst_rect.height() + int_src_rect.height() - 1) / int_src_rect.height();
    return do_draw_integer_scaled_bitmap<has_alpha_channel>(
        target, dst_rect, int_src_rect, source, hfactor, vfactor, get_pixel,
        opacity);
  }

  if constexpr (scaling_mode == Painter::ScalingMode::NearestNeighbor ||
                scaling_mode == Painter::ScalingMode::SmoothPixels) {
    if (dst_rect == clipped_rect && int_src_rect == src_rect &&
        !(dst_rect.width() % int_src_rect.width()) &&
        !(dst_rect.height() % int_src_rect.height())) {
      int hfactor = dst_rect.width() / int_src_rect.width();
      int vfactor = dst_rect.height() / int_src_rect.height();
      if (hfactor == 2 && vfactor == 2)
        return do_draw_integer_scaled_bitmap<has_alpha_channel>(
            target, dst_rect, int_src_rect, source, 2, 2, get_pixel, opacity);
      if (hfactor == 3 && vfactor == 3)
        return do_draw_integer_scaled_bitmap<has_alpha_channel>(
            target, dst_rect, int_src_rect, source, 3, 3, get_pixel, opacity);
      if (hfactor == 4 && vfactor == 4)
        return do_draw_integer_scaled_bitmap<has_alpha_channel>(
            target, dst_rect, int_src_rect, source, 4, 4, get_pixel, opacity);
      return do_draw_integer_scaled_bitmap<has_alpha_channel>(
          target, dst_rect, int_src_rect, source, hfactor, vfactor, get_pixel,
          opacity);
    }
  ...
  i64 bilinear_offset_x =
      (1ll << 31) * (src_rect.width() / dst_rect.width() - 1);
  i64 bilinear_offset_y =
      (1ll << 31) * (src_rect.height() / dst_rect.height() - 1);
  ...
  i64 clipped_src_bottom_shifted =
      (clipped_src_rect.y() + clipped_src_rect.height()) * shift;
  i64 clipped_src_right_shifted =
      (clipped_src_rect.x() + clipped_src_rect.width()) * shift;
  ...
  for (int y = clipped_rect.top(); y <= clipped_rect.bottom(); ++y) {
    ...
    if (desired_y < clipped_src_rect.top() ||
        desired_y > clipped_src_bottom_shifted)
      continue;

    for (int x = clipped_rect.left(); x <= clipped_rect.right(); ++x) {
      auto desired_x = ((x - dst_rect.x()) * hscale + src_left);
      if (desired_x < clipped_src_rect.left() ||
          desired_x > clipped_src_right_shifted)
        continue;
      ...
      if constexpr (scaling_mode == Painter::ScalingMode::BilinearBlend) {
        ...
        auto scaled_x0 = clamp(shifted_x >> 32, clipped_src_rect.left(),
                               clipped_src_rect.right());
        auto scaled_x1 = clamp((shifted_x >> 32) + 1, clipped_src_rect.left(),
                               clipped_src_rect.right());
        auto scaled_y0 = clamp(shifted_y >> 32, clipped_src_rect.top(),
                               clipped_src_rect.bottom());
        auto scaled_y1 = clamp((shifted_y >> 32) + 1, clipped_src_rect.top(),
                               clipped_src_rect.bottom());

        float x_ratio =
            (shifted_x & fractional_mask) / static_cast<float>(shift);
        float y_ratio =
            (shifted_y & fractional_mask) / static_cast<float>(shift);
        ...
        auto scaled_x1 = clamp(desired_x >> 32, clipped_src_rect.left(),
                               clipped_src_rect.right());
        auto scaled_x0 = clamp(scaled_x1 - 1, clipped_src_rect.left(),
                               clipped_src_rect.right());
        auto scaled_y1 = clamp(desired_y >> 32, clipped_src_rect.top(),
                               clipped_src_rect.bottom());
        auto scaled_y0 = clamp(scaled_y1 - 1, clipped_src_rect.top(),
                               clipped_src_rect.bottom());

        float x_ratio = (desired_x & fractional_mask) / (float)shift;
        float y_ratio = (desired_y & fractional_mask) / (float)shift;

        float scaled_x_ratio = clamp(
            x_ratio * dst_rect.width() / (float)src_rect.width(), 0.0f, 1.0f);
        float scaled_y_ratio = clamp(
            y_ratio * dst_rect.height() / (float)src_rect.height(), 0.0f, 1.0f);
        ...
        auto scaled_x = clamp(desired_x >> 32, clipped_src_rect.left(),
                              clipped_src_rect.right());
        auto scaled_y = clamp(desired_y >> 32, clipped_src_rect.top(),
                              clipped_src_rect.bottom());
...
do_draw_scaled_bitmap(Gfx::Bitmap &target, IntRect const &dst_rect,
                      IntRect const &clipped_rect, Gfx::Bitmap const &source,
                      FloatRect const &src_rect, GetPixel get_pixel,
                      float opacity, Painter::ScalingMode scaling_mode) {
  switch (scaling_mode) {
  case Painter::ScalingMode::NearestFractional:
    do_draw_scaled_bitmap<has_alpha_channel,
                          Painter::ScalingMode::NearestFractional>(
        target, dst_rect, clipped_rect, source, src_rect, get_pixel, opacity);
    break;
  case Painter::ScalingMode::NearestNeighbor:
    do_draw_scaled_bitmap<has_alpha_channel,
                          Painter::ScalingMode::NearestNeighbor>(
        target, dst_rect, clipped_rect, source, src_rect, get_pixel, opacity);
    break;
  case Painter::ScalingMode::SmoothPixels:
    do_draw_scaled_bitmap<has_alpha_channel,
                          Painter::ScalingMode::SmoothPixels>(
        target, dst_rect, clipped_rect, source, src_rect, get_pixel, opacity);
    break;
  ...
  case Painter::ScalingMode::None:
    do_draw_scaled_bitmap<has_alpha_channel, Painter::ScalingMode::None>(
        target, dst_rect, clipped_rect, source, src_rect, get_pixel, opacity);
    break;
  }
}

void Painter::draw_scaled_bitmap(IntRect const &a_dst_rect,
                                 Gfx::Bitmap const &source,
                                 IntRect const &a_src_rect, float opacity,
...
void Painter::draw_scaled_bitmap(IntRect const &a_dst_rect,
                                 Gfx::Bitmap const &source,
                                 FloatRect const &a_src_rect, float opacity,
                                 ScalingMode scaling_mode) {
  IntRect int_src_rect = enclosing_int_rect(a_src_rect);
  if (scale() == source.scale() && a_src_rect == int_src_rect &&
      a_dst_rect.size() == int_src_rect.size())
    return blit(a_dst_rect.location(), source, int_src_rect, opacity);

  if (scaling_mode == ScalingMode::None) {
    IntRect clipped_draw_rect{
        (int)a_src_rect.location().x(), (int)a_src_rect.location().y(),
        a_dst_rect.size().width(), a_dst_rect.size().height()};
  ...
  if (source.has_alpha_channel() || opacity != 1.0f) {
    ...
    case BitmapFormat::BGRx8888:
      do_draw_scaled_bitmap<true>(
          *m_target, dst_rect, clipped_rect, source, src_rect,
          Gfx::get_pixel<BitmapFormat::BGRx8888>, opacity, scaling_mode);
      break;
    case BitmapFormat::BGRA8888:
      do_draw_scaled_bitmap<true>(
          *m_target, dst_rect, clipped_rect, source, src_rect,
          Gfx::get_pixel<BitmapFormat::BGRA8888>, opacity, scaling_mode);
      break;
    case BitmapFormat::Indexed8:
      do_draw_scaled_bitmap<true>(
          *m_target, dst_rect, clipped_rect, source, src_rect,
          Gfx::get_pixel<BitmapFormat::Indexed8>, opacity, scaling_mode);
      break;
    case BitmapFormat::Indexed4:
      do_draw_scaled_bitmap<true>(
          *m_target, dst_rect, clipped_rect, source, src_rect,
          Gfx::get_pixel<BitmapFormat::Indexed4>, opacity, scaling_mode);
      break;
    case BitmapFormat::Indexed2:
      do_draw_scaled_bitmap<true>(
          *m_target, dst_rect, clipped_rect, source, src_rect,
          Gfx::get_pixel<BitmapFormat::Indexed2>, opacity, scaling_mode);
      break;
    case BitmapFormat::Indexed1:
      do_draw_scaled_bitmap<true>(
          *m_target, dst_rect, clipped_rect, source, src_rect,
          Gfx::get_pixel<BitmapFormat::Indexed1>, opacity, scaling_mode);
      break;
    default:
      do_draw_scaled_bitmap<true>(
          *m_target, dst_rect, clipped_rect, source, src_rect,
          Gfx::get_pixel<BitmapFormat::Invalid>, opacity, scaling_mode);
      break;
    }
  } else {
    switch (source.format()) {
    case BitmapFormat::BGRx8888:
      do_draw_scaled_bitmap<false>(
          *m_target, dst_rect, clipped_rect, source, src_rect,
          Gfx::get_pixel<BitmapFormat::BGRx8888>, opacity, scaling_mode);
      break;
    case BitmapFormat::Indexed8:
...
FLATTEN void Painter::draw_glyph(FloatPoint point, u32 code_point,
                                 Color color) {
  draw_glyph(point, code_point, font(), color);
}

FLATTEN void Painter::draw_glyph(FloatPoint point, u32 code_point,
                                 Font const &font, Color color) {
  ...
  if (glyph.is_glyph_bitmap()) {
    ...
    blit_filtered(
        glyph_position.blit_position, *glyph.bitmap(), glyph.bitmap()->rect(),
        [color](Color pixel) -> Color { return pixel.multiply(color); });
  }
}

void Painter::draw_emoji(IntPoint point, Gfx::Bitmap const &emoji,
                         Font const &font) {
  dbgln("the computer width for emoji  is {} * {} / {}", font.pixel_size(),
        emoji.width(), emoji.height());
  IntRect dst_rect{
      point.x(),
      point.y(),
      static_cast<int>(
          ceilf(font.pixel_size() * emoji.width() / emoji.height())),
      static_cast<int>(ceilf(font.pixel_size())),
  };
  draw_scaled_bitmap(dst_rect, emoji, emoji.rect());
}

void Painter::draw_glyph_or_emoji(FloatPoint point, u32 code_point,
                                  Font const &font, Color color) {
  StringBuilder builder;
  builder.append_code_point(code_point);
  auto it = Utf8View{builder.string_view()}.begin();
  bool temp = false;
  return draw_glyph_or_emoji(point, it, font, color, temp);
}

// TODO(ftommasi): would be good to know if we "skipped" a codepoint so that we
// dont draw it. i.e. all the emoji specifiers
void Painter::draw_glyph_or_emoji(FloatPoint point, Utf8CodePointIterator &it,
                                  Font const &font, Color color,
                                  bool &skipped_iter) {
  ...
  dbgln("in draw_glyph_or_emoji for {}({},{})", code_point, point.x(),
        point.y());
  ScopeGuard consume_variation_selector = [&] {
    // If we advanced the iterator to consume an emoji sequence, don't look for
    // another variation selector.
    if (initial_it != it) {
      skipped_iter = true;
      return;
    }
    ...
    auto next_code_point = it.peek(1);
    if (next_code_point == text_variation_selector ||
  ...
  auto code_point_is_regional_indicator =
      code_point >= regional_indicator_symbol_a &&
      code_point <= regional_indicator_symbol_z;
  auto font_contains_glyph = font.contains_glyph(code_point);

  auto check_for_emoji =
      false
      ...
      // keycap emojis ({#,*,0-9} U+FE0F U+20E3) start with a regular ASCII
      // character. Both cases are handled by peeking for the variation
      // selector.
      || next_code_point == emoji_variation_selector;

  // If the font contains the glyph, and we know it's not the start of an emoji,
  // draw a text glyph.
  if (font_contains_glyph && !check_for_emoji) {
    dbgln("in draw_glyph for {}", code_point);
    draw_glyph(point, code_point, font, color);
    return;
  }

  // If we didn't find a text glyph, or have an emoji variation selector or
  // regional indicator, try to draw an emoji glyph.
  if (auto const *emoji = Emoji::emoji_for_code_point_iterator(it)) {
    dbgln("in draw_emoji for {}", code_point);
  ...
  if (font_contains_glyph) {
    dbgln("after draw_emoji fallback");
    draw_glyph(point, code_point, font, color);
    return;
  }

  // No suitable glyph found, draw a replacement character.
  dbgln_if(EMOJI_DEBUG, "Failed to find a glyph or emoji for code_point {}",
           code_point);
  dbgln("final fallback");
...
void Painter::draw_glyph(IntPoint point, u32 code_point, Font const &font,
                         Color color) {
  draw_glyph(point.to_type<float>(), code_point, font, color);
}

void Painter::draw_glyph_or_emoji(IntPoint point, u32 code_point,
                                  Font const &font, Color color) {
  draw_glyph_or_emoji(point.to_type<float>(), code_point, font, color);
}

void Painter::draw_glyph_or_emoji(IntPoint point, Utf8CodePointIterator &it,
                                  Font const &font, Color color) {
  bool temp = false;
  draw_glyph_or_emoji(point.to_type<float>(), it, font, color, temp);
}

template <typename DrawGlyphFunction>
void draw_text_line(FloatRect const &a_rect, Utf8View const &text,
                    Font const &font, TextAlignment alignment,
                    TextDirection direction, DrawGlyphFunction draw_glyph) {
  auto rect = a_rect;
  ...
  for (auto it = text.begin(); it != text.end(); ++it) {
    ...
    if (kerning != 0.0f)
      point.translate_by(direction == TextDirection::LTR ? kerning : -kerning,
                         0);

    FloatSize glyph_size(font.glyph_or_emoji_width(code_point) +
                             font.glyph_spacing(),
                         font.pixel_size());
    if (direction == TextDirection::RTL)
      point.translate_by(-glyph_size.width(),
                         0); // If we are drawing right to left, we have to move
                             // backwards before drawing the glyph
...
Painter::split_text_into_directional_runs(Utf8View const &text,
                                          TextDirection initial_direction) {
  // FIXME: This is a *very* simplified version of the UNICODE BIDIRECTIONAL
  // ALGORITHM (https://www.unicode.org/reports/tr9/), that can render most
  // bidirectional text
  //  but also produces awkward results in a large amount of edge cases. This
  //  should probably be replaced with a fully spec compliant implementation at
  //  some point.

  // FIXME: Support HTML "dir" attribute (how?)
  u8 paragraph_embedding_level =
      initial_direction == TextDirection::LTR ? 0 : 1;
  ...
  BidirectionalClass paragraph_class = initial_direction == TextDirection::LTR
                                           ? BidirectionalClass::STRONG_LTR
                                           : BidirectionalClass::STRONG_RTL;
  for (size_t i = 0; i < character_classes.size(); i++) {
    ...
    for (ssize_t j = i - 1; j >= 0; j--) {
      ...
      if (character_class != BidirectionalClass::STRONG_RTL &&
          character_class != BidirectionalClass::STRONG_LTR)
        continue;
  ...
  for (size_t i = 0; i < character_classes.size(); i++) {
    ...
    if (character_class != BidirectionalClass::NEUTRAL) {
      ...
      if (bidi_class_to_direction(left_side) ==
          bidi_class_to_direction(character_class)) {
        sequence_class = left_side == BidirectionalClass::STRONG_RTL
                             ? BidirectionalClass::STRONG_RTL
                             : BidirectionalClass::STRONG_LTR;
      } else {
  ...
  for (size_t i = 0; i < character_classes.size(); i++) {
    ...
    if ((embedding_levels[i] % 2) == 0) {
      ...
      else if (character_class == BidirectionalClass::WEAK_NUMBERS ||
               character_class == BidirectionalClass::WEAK_SEPARATORS)
        embedding_levels[i] += 2;
    } else {
      if (character_class == BidirectionalClass::STRONG_LTR ||
          character_class == BidirectionalClass::WEAK_NUMBERS ||
          character_class == BidirectionalClass::WEAK_SEPARATORS)
        embedding_levels[i] += 1;
    }
  }
  ...
  auto code_points_slice =
      next_code_points_slice(embedding_levels.size() - start);
  runs.append({move(code_points_slice), level});
  ...
  while (maximum_level <= minimum_level) {
    ...
    while (run_index < runs_count) {
      while (run_index < runs_count &&
             runs[run_index].embedding_level() < maximum_level)
        run_index++;
...
bool Painter::text_contains_bidirectional_text(
    Utf8View const &text, TextDirection initial_direction) {
  for (u32 code_point : text) {
...
template <typename DrawGlyphFunction>
void Painter::do_draw_text(FloatRect const &rect, Utf8View const &text,
                           Font const &font, TextAlignment alignment,
                           TextElision elision, TextWrapping wrapping,
                           DrawGlyphFunction draw_glyph) {
  if (draw_text_get_length(text) == 0)
    return;
  // TODO(ftommasi): this is where text editor processes
  ...
  for (size_t i = 0; i < lines.size(); ++i) {
    ...
    FloatRect line_rect{bounding_rect.x(), bounding_rect.y() + i * line_height,
                        bounding_rect.width(), line_height};

    TextDirection line_direction = get_text_direction(line);
    if (text_contains_bidirectional_text(
            line, line_direction)) { // Slow Path: The line contains mixed BiDi
                                     // classes
      auto directional_runs =
          split_text_into_directional_runs(line, line_direction);
      auto current_dx =
          line_direction == TextDirection::LTR ? 0 : line_rect.width();
      for (auto &directional_run : directional_runs) {
        ...
        draw_text_line(run_rect, line_text, font, alignment,
                       directional_run.direction(), draw_glyph);
        if (line_direction == TextDirection::LTR)
          current_dx += run_width;
      }
    } else {
      draw_text_line(line_rect, line, font, alignment, line_direction,
                     draw_glyph);
    }
  }
}

void Painter::draw_text(FloatRect const &rect, StringView text,
                        TextAlignment alignment, Color color,
                        TextElision elision, TextWrapping wrapping) {
  draw_text(rect, text, font(), alignment, color, elision, wrapping);
}

void Painter::draw_text(FloatRect const &rect, Utf32View const &text,
                        TextAlignment alignment, Color color,
                        TextElision elision, TextWrapping wrapping) {
  draw_text(rect, text, font(), alignment, color, elision, wrapping);
}

void Painter::draw_text(FloatRect const &rect, StringView raw_text,
                        Font const &font, TextAlignment alignment, Color color,
                        TextElision elision, TextWrapping wrapping) {
  Utf8View text{raw_text};
  do_draw_text(rect, text, font, alignment, elision, wrapping,
               [&](FloatRect const &r, Utf8CodePointIterator &it) {
                 bool temp = false;
                 draw_glyph_or_emoji(r.location(), it, font, color, temp);
               });
}

void Painter::draw_text(FloatRect const &rect, Utf32View const &raw_text,
                        Font const &font, TextAlignment alignment, Color color,
                        TextElision elision, TextWrapping wrapping) {
  ...
  // TODO(ftommasi): this is called by  text editor
  do_draw_text(rect, text, font, alignment, elision, wrapping,
               [&](FloatRect const &r, Utf8CodePointIterator &it) {
                 bool temp = false;
                 draw_glyph_or_emoji(r.location(), it, font, color, temp);
...
void Painter::draw_text(
    Function<void(FloatRect const &, Utf8CodePointIterator &)> draw_one_glyph,
    FloatRect const &rect, Utf8View const &text, Font const &font,
    TextAlignment alignment, TextElision elision, TextWrapping wrapping) {
  VERIFY(scale() == 1); // FIXME: Add scaling support.
...
void Painter::draw_text(
    Function<void(FloatRect const &, Utf8CodePointIterator &)> draw_one_glyph,
    FloatRect const &rect, StringView raw_text, Font const &font,
    TextAlignment alignment, TextElision elision, TextWrapping wrapping) {
  VERIFY(scale() == 1); // FIXME: Add scaling support.
...
void Painter::draw_text(
    Function<void(FloatRect const &, Utf8CodePointIterator &)> draw_one_glyph,
    FloatRect const &rect, Utf32View const &raw_text, Font const &font,
    TextAlignment alignment, TextElision elision, TextWrapping wrapping) {
  VERIFY(scale() == 1); // FIXME: Add scaling support.
...
void Painter::draw_text(IntRect const &rect, StringView text,
                        TextAlignment alignment, Color color,
                        TextElision elision, TextWrapping wrapping) {
  draw_text(rect.to_type<float>(), text, font(), alignment, color, elision,
            wrapping);
}

void Painter::draw_text(IntRect const &rect, Utf32View const &text,
                        TextAlignment alignment, Color color,
                        TextElision elision, TextWrapping wrapping) {
  draw_text(rect.to_type<float>(), text, font(), alignment, color, elision,
            wrapping);
...
void Painter::draw_text(IntRect const &rect, StringView raw_text,
                        Font const &font, TextAlignment alignment, Color color,
                        TextElision elision, TextWrapping wrapping) {
  draw_text(rect.to_type<float>(), raw_text, font, alignment, color, elision,
            wrapping);
}

void Painter::draw_text(IntRect const &rect, Utf32View const &raw_text,
                        Font const &font, TextAlignment alignment, Color color,
                        TextElision elision, TextWrapping wrapping) {
  return draw_text(rect.to_type<float>(), raw_text, font, alignment, color,
                   elision, wrapping);
}

void Painter::draw_text(
    Function<void(FloatRect const &, Utf8CodePointIterator &)> draw_one_glyph,
    IntRect const &rect, Utf8View const &text, Font const &font,
    TextAlignment alignment, TextElision elision, TextWrapping wrapping) {
  return draw_text(move(draw_one_glyph), rect.to_type<float>(), text, font,
                   alignment, elision, wrapping);
}

void Painter::draw_text(
    Function<void(FloatRect const &, Utf8CodePointIterator &)> draw_one_glyph,
    IntRect const &rect, StringView raw_text, Font const &font,
    TextAlignment alignment, TextElision elision, TextWrapping wrapping) {
  return draw_text(move(draw_one_glyph), rect.to_type<float>(), raw_text, font,
                   alignment, elision, wrapping);
}

void Painter::draw_text(
    Function<void(FloatRect const &, Utf8CodePointIterator &)> draw_one_glyph,
    IntRect const &rect, Utf32View const &raw_text, Font const &font,
    TextAlignment alignment, TextElision elision, TextWrapping wrapping) {
  return draw_text(move(draw_one_glyph), rect.to_type<float>(), raw_text, font,
                   alignment, elision, wrapping);
}
...
void Painter::set_pixel(IntPoint p, Color color, bool blend) {
  ...
  // Use the scale only to avoid clipping pixels set in drawing functions that
  // handle scaling and call set_pixel() -- do not scale the pixel.
  if (!clip_rect().contains(point / scale()))
    return;
  set_physical_pixel(point, color, blend);
}

void Painter::set_physical_pixel(IntPoint physical_point, Color color,
                                 bool blend) {
  // This function should only be called after translation, clipping, etc has
  // been handled elsewhere if not use set_pixel().
...
ErrorOr<NonnullRefPtr<Bitmap>>
Painter::get_region_bitmap(IntRect const &region, BitmapFormat format,
                           Optional<IntRect &> actual_region) {
  VERIFY(scale() == 1);
  auto bitmap_region =
      region.translated(state().translation).intersected(m_target->rect());
...
ALWAYS_INLINE void Painter::set_physical_pixel_with_draw_op(u32 &pixel,
                                                            Color color) {
...
ALWAYS_INLINE void Painter::fill_physical_scanline_with_draw_op(int y, int x,
                                                                int width,
                                                                Color color) {
...
void Painter::draw_physical_pixel(IntPoint physical_position, Color color,
                                  int thickness) {
  ...
  if (thickness == 1) { // Implies scale() == 1.
    auto &pixel =
        m_target->scanline(physical_position.y())[physical_position.x()];
    return set_physical_pixel_with_draw_op(
        pixel, Color::from_argb(pixel).blend(color));
  }
...
void Painter::draw_line(IntPoint a_p1, IntPoint a_p2, Color color,
                        int thickness, LineStyle style, Color alternate_color) {
  if (thickness <= 0)
    return;
  ...
  auto const p1 = thickness > 1
                      ? a_p1.translated(-(thickness / 2), -(thickness / 2))
                      : a_p1;
  auto const p2 = thickness > 1
                      ? a_p2.translated(-(thickness / 2), -(thickness / 2))
                      : a_p2;
  ...
  if (point1.x() == point2.x()) {
    ...
    if (style == LineStyle::Dotted) {
      ...
      for (int y = min_y; y <= max_y; y += thickness * 6) {
        ...
        draw_physical_pixel({x, min(y + thickness * 2, max_y)}, color,
                            thickness);
        if (!alternate_color_is_transparent) {
          draw_physical_pixel({x, min(y + thickness * 3, max_y)},
                              alternate_color, thickness);
          draw_physical_pixel({x, min(y + thickness * 4, max_y)},
                              alternate_color, thickness);
          draw_physical_pixel({x, min(y + thickness * 5, max_y)},
                              alternate_color, thickness);
        }
      }
    } else {
  ...
  if (point1.y() == point2.y()) {
    ...
    if (style == LineStyle::Dotted) {
      ...
      for (int x = min_x; x <= max_x; x += thickness * 6) {
        ...
        draw_physical_pixel({min(x + thickness * 2, max_x), y}, color,
                            thickness);
        if (!alternate_color_is_transparent) {
          draw_physical_pixel({min(x + thickness * 3, max_x), y},
                              alternate_color, thickness);
          draw_physical_pixel({min(x + thickness * 4, max_x), y},
                              alternate_color, thickness);
          draw_physical_pixel({min(x + thickness * 5, max_x), y},
                              alternate_color, thickness);
        }
      }
    } else {
...
void Painter::draw_triangle_wave(IntPoint a_p1, IntPoint a_p2, Color color,
                                 int amplitude, int thickness) {
...
static bool can_approximate_bezier_curve(FloatPoint p1, FloatPoint p2,
                                         FloatPoint control) {
...
// static
void Painter::for_each_line_segment_on_bezier_curve(
    FloatPoint control_point, FloatPoint p1, FloatPoint p2,
    Function<void(FloatPoint, FloatPoint)> &callback) {
  ...
  static constexpr auto split_quadratic_bezier_curve =
      [](FloatPoint original_control, FloatPoint p1, FloatPoint p2,
         auto &segments) {
  ...
  while (!segments.is_empty()) {
    ...
    if (can_approximate_bezier_curve(segment.p1, segment.p2,
                                     segment.control_point))
      callback(segment.p1, segment.p2);
    else
      split_quadratic_bezier_curve(segment.control_point, segment.p1,
                                   segment.p2, segments);
  }
}
...
void Painter::draw_quadratic_bezier_curve(IntPoint control_point, IntPoint p1,
                                          IntPoint p2, Color color,
                                          int thickness, LineStyle style) {
  VERIFY(scale() == 1); // FIXME: Add scaling support.

  if (thickness <= 0)
    return;

  for_each_line_segment_on_bezier_curve(
      FloatPoint(control_point), FloatPoint(p1), FloatPoint(p2),
      [&](FloatPoint fp1, FloatPoint fp2) {
        draw_line(IntPoint(fp1.x(), fp1.y()), IntPoint(fp2.x(), fp2.y()), color,
                  thickness, style);
      });
}

void Painter::for_each_line_segment_on_cubic_bezier_curve(
    FloatPoint control_point_0, FloatPoint control_point_1, FloatPoint p1,
    FloatPoint p2, Function<void(FloatPoint, FloatPoint)> &&callback) {
  for_each_line_segment_on_cubic_bezier_curve(control_point_0, control_point_1,
                                              p1, p2, callback);
}

static bool can_approximate_cubic_bezier_curve(FloatPoint p1, FloatPoint p2,
                                               FloatPoint control_0,
                                               FloatPoint control_1) {
  constexpr float tolerance = 0.0015f;
...
void Painter::for_each_line_segment_on_cubic_bezier_curve(
  ...
  static constexpr auto split_cubic_bezier_curve =
      [](ControlPair const &original_controls, FloatPoint p1, FloatPoint p2,
         auto &segments) {
        Array level_1_midpoints{
            (p1 + original_controls.control_point_0) / 2,
            (original_controls.control_point_0 +
             original_controls.control_point_1) /
                2,
        ...
        auto level_3_midpoint =
            (level_2_midpoints[0] + level_2_midpoints[1]) / 2;

        segments.enqueue({{level_1_midpoints[0], level_2_midpoints[0]},
                          p1,
                          level_3_midpoint});
        segments.enqueue({{level_2_midpoints[1], level_1_midpoints[2]},
                          level_3_midpoint,
                          p2});
      };
  ...
  while (!segments.is_empty()) {
    ...
    if (can_approximate_cubic_bezier_curve(
            segment.p1, segment.p2, segment.control_points.control_point_0,
            segment.control_points.control_point_1))
      callback(segment.p1, segment.p2);
    else
      split_cubic_bezier_curve(segment.control_points, segment.p1, segment.p2,
                               segments);
  }
}

void Painter::draw_cubic_bezier_curve(IntPoint control_point_0,
                                      IntPoint control_point_1, IntPoint p1,
                                      IntPoint p2, Color color, int thickness,
                                      Painter::LineStyle style) {
  for_each_line_segment_on_cubic_bezier_curve(
      FloatPoint(control_point_0), FloatPoint(control_point_1), FloatPoint(p1),
      FloatPoint(p2), [&](FloatPoint fp1, FloatPoint fp2) {
        draw_line(IntPoint(fp1.x(), fp1.y()), IntPoint(fp2.x(), fp2.y()), color,
                  thickness, style);
      });
}
...
// static
void Painter::for_each_line_segment_on_elliptical_arc(
    FloatPoint p1, FloatPoint p2, FloatPoint center, FloatPoint const radii,
    float x_axis_rotation, float theta_1, float theta_delta,
    Function<void(FloatPoint, FloatPoint)> &callback) {
  ...
  for (float theta = theta_1; theta <= theta_1 + theta_delta;
       theta += theta_step) {
    float s, c;
    AK::sincos(theta, s, c);
...
void Painter::for_each_line_segment_on_elliptical_arc(
    ...
    float x_axis_rotation, float theta_1, float theta_delta,
    Function<void(FloatPoint, FloatPoint)> &&callback) {
  for_each_line_segment_on_elliptical_arc(
      p1, p2, center, radii, x_axis_rotation, theta_1, theta_delta, callback);
}

void Painter::draw_elliptical_arc(IntPoint p1, IntPoint p2, IntPoint center,
                                  FloatPoint radii, float x_axis_rotation,
                                  float theta_1, float theta_delta, Color color,
                                  int thickness, LineStyle style) {
  VERIFY(scale() == 1); // FIXME: Add scaling support.
  ...
  for_each_line_segment_on_elliptical_arc(
      FloatPoint(p1), FloatPoint(p2), FloatPoint(center), radii,
      x_axis_rotation, theta_1, theta_delta,
      [&](FloatPoint fp1, FloatPoint fp2) {
        draw_line(IntPoint(fp1.x(), fp1.y()), IntPoint(fp2.x(), fp2.y()), color,
                  thickness, style);
      });
...
void Painter::add_clip_rect(IntRect const &rect) {
  ...
  state().clip_rect.intersect(
      m_target->rect()); // FIXME: This shouldn't be necessary?
}

void Painter::clear_clip_rect() { state().clip_rect = m_clip_origin; }

PainterStateSaver::PainterStateSaver(Painter &painter) : m_painter(painter) {
  m_painter.save();
}

PainterStateSaver::~PainterStateSaver() { m_painter.restore(); }
...
void Painter::stroke_path(Path const &path, Color color, int thickness) {
  ...
  for (auto &segment : path.segments()) {
    ...
    case Segment::Type::LineTo:
      draw_line(cursor.to_type<int>(), segment.point().to_type<int>(), color,
                thickness);
      cursor = segment.point();
      break;
    case Segment::Type::QuadraticBezierCurveTo: {
      auto through =
          static_cast<QuadraticBezierCurveSegment const &>(segment).through();
      draw_quadratic_bezier_curve(through.to_type<int>(), cursor.to_type<int>(),
                                  segment.point().to_type<int>(), color,
                                  thickness);
      cursor = segment.point();
      break;
    }
    case Segment::Type::CubicBezierCurveTo: {
      ...
      draw_cubic_bezier_curve(through_0.to_type<int>(),
                              through_1.to_type<int>(), cursor.to_type<int>(),
                              segment.point().to_type<int>(), color, thickness);
      cursor = segment.point();
      break;
    }
    case Segment::Type::EllipticalArcTo:
      auto &arc = static_cast<EllipticalArcSegment const &>(segment);
      draw_elliptical_arc(cursor.to_type<int>(), segment.point().to_type<int>(),
                          arc.center().to_type<int>(), arc.radii(),
                          arc.x_axis_rotation(), arc.theta_1(),
                          arc.theta_delta(), color, thickness);
      cursor = segment.point();
      break;
    }
  }
...
void Painter::fill_path(Path const &path, Color color,
...
void Painter::fill_path(Path const &path, PaintStyle const &paint_style,
                        Painter::WindingRule rule) {
  VERIFY(scale() == 1); // FIXME: Add scaling support.
  paint_style.paint(enclosing_int_rect(path.bounding_box()),
                    [&](PaintStyle::SamplerFunction sampler) {
                      Detail::fill_path<Detail::FillPathMode::PlaceOnIntGrid>(
                          *this, path, move(sampler), rule);
                    });
}

void Painter::blit_disabled(IntPoint location, Gfx::Bitmap const &bitmap,
                            IntRect const &rect, Palette const &palette) {
  auto bright_color = palette.threed_highlight();
  auto dark_color = palette.threed_shadow1();
  blit_filtered(location.translated(1, 1), bitmap, rect,
                [&](auto) { return bright_color; });
...
void Painter::blit_tiled(IntRect const &dst_rect, Gfx::Bitmap const &bitmap,
                         IntRect const &rect) {
  auto tile_width = rect.width();
  auto tile_height = rect.height();
  auto dst_right = dst_rect.right();
  auto dst_bottom = dst_rect.bottom();
  for (int tile_y = dst_rect.top(); tile_y < dst_bottom;
       tile_y += tile_height) {
    for (int tile_x = dst_rect.left(); tile_x < dst_right;
         tile_x += tile_width) {
...
DeprecatedString parse_ampersand_string(StringView raw_text,
...
void Gfx::Painter::draw_ui_text(Gfx::IntRect const &rect, StringView text,
                                Gfx::Font const &font,
  ...
  Gfx::IntRect text_rect{0, 0,
                         static_cast<int>(ceilf(font.width(name_to_draw))),
                         static_cast<int>(ceilf(font.pixel_size()))};
...
void Painter::draw_text_run(IntPoint baseline_start, Utf8View const &string,
                            Font const &font, Color color) {
  draw_text_run(baseline_start.to_type<float>(), string, font, color);
}

void Painter::draw_text_run(FloatPoint baseline_start, Utf8View const &string,
                            Font const &font, Color color) {
  ...
  for (auto code_point_iterator = string.begin();
       code_point_iterator != string.end(); ++code_point_iterator) {
    ...
    dbgln("calling draw_text_run loop iter : {}", code_point_counter);
    ++code_point_counter;
    if (should_paint_as_space(code_point)) {
      dbgln("in should_paint_as_space loop iter : {}", code_point_counter);
      x += space_width + font.glyph_spacing();
      last_code_point = code_point;
      continue;
    }

    // TODO(ftommasi): this is maybe where the issue is (i dont think)
    //  FIXME: this is probably not the real space taken for complex emojis
    // x += font.glyphs_horizontal_kerning(first_code_point,
    // second_code_point);
    x += font.glyphs_horizontal_kerning(last_code_point, code_point);
    dbgln("after glyphs_horiz_kern x is {} during iter : {}", x,
          code_point_counter);
    bool skipped_iter = false;
    ...
    dbgln("after glyphs_or_emoji_width x is {} during iter : {}", x,
          code_point_counter);
...
void Painter::draw_scaled_bitmap_with_transform(
    IntRect const &dst_rect, Bitmap const &bitmap, FloatRect const &src_rect,
    AffineTransform const &transform, float opacity,
  ...
  if (transform.is_identity_or_translation()) {
    ...
    // - For each point in the computed bounding rect, reverse-map it to a point
    // in the source image.
    //   - Sample the source image at the computed point.
    //   - Set or blend (depending on alpha values) one pixel in the canvas.
    //   - Loop.

    // FIXME: Painter should have an affine transform as part of its state and
    ...
    auto destination_bounding_rect =
        destination_quad.bounding_rect().to_rounded<int>();

    Gfx::AffineTransform source_transform;
    source_transform.translate(src_rect.x(), src_rect.y());
    source_transform.scale(src_rect.width() / dst_rect.width(),
                           src_rect.height() / dst_rect.height());
    source_transform.translate(-dst_rect.x(), -dst_rect.y());

    for (int y = destination_bounding_rect.y();
         y <= destination_bounding_rect.bottom(); ++y) {
      for (int x = destination_bounding_rect.x();
           x <= destination_bounding_rect.right(); ++x) {
        ...
        if (!destination_quad.contains(destination_point.to_type<float>()))
          continue;
        auto source_point =
            source_transform.map(inverse_transform->map(destination_point))
                .to_rounded<int>();
