...
ScaledFont::ScaledFont(NonnullRefPtr<VectorFont> font, float point_width,
                       float point_height, unsigned dpi_x, unsigned dpi_y)
    : m_font(move(font)), m_point_width(point_width),
...
float ScaledFont::width(StringView view) const {
  return unicode_view_width(Utf8View(view));
}
float ScaledFont::width(Utf8View const &view) const {
  return unicode_view_width(view);
}
float ScaledFont::width(Utf32View const &view) const {
  return unicode_view_width(view);
}

// TODO(ftommasi): this function tells Painter class how long the line is for
// line_run when do draw text
template <typename T>
ALWAYS_INLINE float ScaledFont::unicode_view_width(T const &view) const {
  ...
  for (auto code_point : view) {
    ...
    dbgln("glyphid for  code_point {} = {}", code_point, glyph_id);
    auto kerning = glyphs_horizontal_kerning(last_code_point, code_point);
    // if(glyph_id == ???) continue; //We skip the "invisible" glyph width
...
RefPtr<Gfx::Bitmap>
ScaledFont::rasterize_glyph(u32 glyph_id,
                            GlyphSubpixelOffset subpixel_offset) const {
  ...
  auto glyph_bitmap =
      m_font->rasterize_glyph(glyph_id, m_x_scale, m_y_scale, subpixel_offset);
...
Gfx::Glyph ScaledFont::glyph(u32 code_point,
                             GlyphSubpixelOffset subpixel_offset) const {
  auto id = glyph_id_for_code_point(code_point);
  auto bitmap = rasterize_glyph(id, subpixel_offset);
  auto metrics = glyph_metrics(id);
  return Gfx::Glyph(bitmap, metrics.left_side_bearing, metrics.advance_width,
                    metrics.ascender);
}
...
float ScaledFont::glyph_width(u32 code_point) const {
  // TODO(ftommasi): mauybe "invisible" codepoints should have 0 width
  auto id = glyph_id_for_code_point(code_point);
  auto metrics = glyph_metrics(id);
...
float ScaledFont::glyphs_horizontal_kerning(u32 left_code_point,
                                            u32 right_code_point) const {
  ...
  return m_font->glyphs_horizontal_kerning(left_glyph_id, right_glyph_id,
                                           m_x_scale);
}
